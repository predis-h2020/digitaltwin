...
# ts_data = np.array(
#     [0, 8, 14, 19, 26, 46, 84, 123, 140, 173, 197, 200, 235, 252, 264, 284, 322, 361]
# )
# epss_data = [
#     0,
#     0.01,
...
def zeta_func(t, Tau_c, Tau_L):
    ...
    if return_jacobian:
        d1 = -t * a1 / (Tau_c**2 * (a2 + 1.0))
        d1 += (1.0 - a1) * (-t + Tau_L) * a2 / (Tau_c**2 * (a2 + 1.0) ** 2)
        d2 = -(1.0 - a1) * a2 / (Tau_c * (a2 + 1.0) ** 2)
        return (1.0 - a1) / (1.0 + a2), np.array([d1, d2])
    else:
...
for i, n in enumerate(pars_names):
    ...
    plot_pdfs(
        ...
        colors=[options_prior["color"], options_posterior["color"]],
        tit=f"Parameter {n}",
        xl=n,
        _path=_path,
        _name=f"Parameter_{n}",
        _format=".png",
        dpi=300,
        sz=16,
    )
...
plot_pdfs(
    ...
    tit=f"Inference of precision of noise",
    xl="$\phi$",
    _path=_path,
    _name=f"Noise_precision",
    _format=".png",
    dpi=300,
    sz=16,
)
...
if return_jacobian:
    ...
    f_at_posterior, diff = eps_total(
        ts_data, eps_inf=post_m[0], Tau_c=post_m[1], Tau_L=post_m[2]
    )
    ...
    cov_at_noise_mean = 1.0 / vb_results.noise.mean * np.linalg.inv(cov_inv)
    # Sum
    predicted_std_at_post = np.diag(cov_propagated + cov_at_noise_mean) ** (0.5)
else:
    f_at_posterior = eps_total(
        ts_data, eps_inf=post_m[0], Tau_c=post_m[1], Tau_L=post_m[2]
    )
    print(
        f"Propagation of parameter variability into the model is ignored, since no jacobian is provided."
    )
...
plt.plot(
    ...
    fillstyle="none",
    label="model (mean)\n(VB-optimized)",
    color=options_predict["color"],
)
plt.fill_between(
    ts_data,
    f_at_posterior - 2 * predicted_std_at_post,
    f_at_posterior + 2 * predicted_std_at_post,
    label="model (4$\sigma$)\n(VB-optimized)",
    alpha=0.10,
    **{"color": options_predict["color"]},
