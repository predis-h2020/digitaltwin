...
def plot_pdfs(dists, labels, colors=None, interval_prob=0.99, target=None \
              , tit='', xl='', yl='pdf', _path='', _name='pdf plot', _format='.png', dpi=300, sz=16, _show_values=True):
    """
    ...
    if target is not None:
        if type(target)==list or type(target)==np.ndarray:
            for i, t in enumerate(target):
                ...
                plt.bar(x=t, height=1.0*max_pdf, width=max_std*0.02, color=PltOptions['perfect']['color'])
                if _show_values:
            ...
            plt.bar(x=target, height=1.0*max_pdf, width=max_std*0.03, color=PltOptions['perfect']['color'])
            if _show_values:
...
def gaussian_error_propagator(jac, stds=None, cov=None, rtol=1e-12, atol=1e-6):
    ...
    if (stds is None) and (cov is None):
        ...
        if (stds is not None) and (cov is not None): # if both 'stds' 'cov' are given, we check them to agree.
            e = np.linalg.norm(np.diag(cov)**(0.5) - stds) / np.linalg.norm(stds)
            if e > rtol:
                raise ValueError(f"The diagonal values of the 'cov' do not agree with the 'stds'.")
